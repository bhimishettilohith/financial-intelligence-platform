...
def load_core_file(file_name):
    ...
    """
    return pd.read_excel(RAW_DATA_PATH / file_name, header=1)
...
def dq02_company_year_uniqueness(pl_df):
    ...
    duplicates = pl_df[pl_df.duplicated(subset=["company_id", "year"], keep=False)]
...
def dq04_balance_sheet_balance(bs_df):
    ...
    valid_rows = bs_df[bs_df["total_assets"] > 0].copy()

    valid_rows["diff_pct"] = (
        abs(valid_rows["total_assets"] - valid_rows["total_liabilities"])
        / valid_rows["total_assets"]
        * 100
    )

    failures = valid_rows[valid_rows["diff_pct"] > 1]
...
def dq05_opm_crosscheck(pl_df):
    ...
    valid_rows["calc_opm"] = valid_rows["operating_profit"] / valid_rows["sales"] * 100

    valid_rows["diff"] = abs(valid_rows["calc_opm"] - valid_rows["opm_percentage"])

    failures = valid_rows[valid_rows["diff"] > 2]
...
def dq07_net_cashflow(cf_df):
    ...
    failures = cf_df[abs(calc - cf_df["net_cash_flow"]) > 1]
...
def dq08_tax_rate(pl_df):
    ...
    failures = pl_df[(pl_df["tax_percentage"] < 0) | (pl_df["tax_percentage"] > 100)]
...
def dq09_dividend_payout(pl_df):
    ...
    failures = pl_df[(pl_df["dividend_payout"] < 0) | (pl_df["dividend_payout"] > 100)]
...
def dq11_documents_fk(doc_df, valid_company_ids):
    ...
    failures = doc_df[~doc_df["company_id"].isin(valid_company_ids)]
...
def dq12_analysis_fk(analysis_df, valid_company_ids):
    ...
    failures = analysis_df[~analysis_df["company_id"].isin(valid_company_ids)]
...
def dq13_proscons_fk(pc_df, valid_company_ids):
    ...
    failures = pc_df[~pc_df["company_id"].isin(valid_company_ids)]
...
def dq14_url_validation(doc_df):
    ...
    failures = doc_df[
        ~doc_df["Annual_Report"].astype(str).str.startswith(("http://", "https://"))
    ]
...
def dq15_duplicate_reports(doc_df):
    ...
    failures = doc_df[doc_df.duplicated(subset=["company_id", "Year"], keep=False)]
...
def dq16_missing_values(companies_df):
    ...
    failures = companies_df[companies_df.isnull().any(axis=1)]
...
if __name__ == "__main__":
    ...
    # DQ-02
    dq02_company_year_uniqueness(profitandloss)

    # DQ-03
    dq03_fk_integrity(profitandloss, "profitandloss", valid_company_ids)

    dq03_fk_integrity(balancesheet, "balancesheet", valid_company_ids)

    dq03_fk_integrity(cashflow, "cashflow", valid_company_ids)

    # DQ-04
    dq04_balance_sheet_balance(balancesheet)

    # DQ-05
    dq05_opm_crosscheck(profitandloss)

    # DQ-06
    dq06_positive_sales(profitandloss)

    # DQ-07
    dq07_net_cashflow(cashflow)

    # DQ-08
    dq08_tax_rate(profitandloss)

    # DQ-09
    dq09_dividend_payout(profitandloss)

    # DQ-10
    dq10_eps_sign(profitandloss)

    # DQ-11
    dq11_documents_fk(documents, valid_company_ids)

    # DQ-12
    dq12_analysis_fk(analysis, valid_company_ids)

    # DQ-13
    dq13_proscons_fk(prosandcons, valid_company_ids)

    # DQ-14
    dq14_url_validation(documents)

    # DQ-15
    dq15_duplicate_reports(documents)

    # DQ-16
    dq16_missing_values(companies)

    failures_df = pd.DataFrame(
        validation_failures,
        columns=["rule_id", "severity", "table", "company", "message"],
    )
    ...
    print(f"\nValidation complete. " f"Failures: {len(failures_df)}")
